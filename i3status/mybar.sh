...
myvpn_on() {
  ...
  separator $bg $bg_bar_color # background left previous block
...
disk_usage() {
  ...
  echo -n "\"full_text\":\"  $(df / | tail -1 | awk '{print $5}')\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "}"
}
...
#  echo -n "\"full_text\":\"  $(/home/you/.config/i3status/memory.py)%\","
  echo -n "\"full_text\":\"  $(vmstat -ay 1 2 | tail -1 | awk -F' ' '{printf("%2d", (8 - $4 / 1024 / 1024) / 8 * 100)}')%\","
...
cpu_usage() {
  ...
  echo -n "\"full_text\":\"  $(cat /proc/loadavg | awk '{print $1 * 100}')% \","
  echo -n "\"background\":\"#3949AB\","
  common
  echo -n "},"
}
...
meteo() {
  ...
  echo -n "\"full_text\": \"$(curl -s https://wttr.in/?format=%c%t) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
volume() {
  local bg="#673AB7"
  local bg_separator_previous="#546E7A"
  separator $bg $bg_separator_previous  
...
mydate() {
  ...
  echo -n ",{"
  echo -n "\"name\":\"id_time\","
  echo -n "\"full_text\":\"   $(date "+%a %d %b %H:%M") \","
  echo -n "\"color\":\"#000000\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
...
systemupdate() {
  local nb=$(echo 1)
  local bg="#2E7D32" 
  local bg_separator_previous="#673AB7"
  separator $bg $bg_separator_previous
  if [[ $nb > 0 ]]; then
    echo -n ",{"
...
while read line;
do
  # echo $line > /home/stas/.config/i3/tmp.txt
